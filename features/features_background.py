...
def QASYM(EEG, channels):
    ...
    return float(np.sum(np.abs(asymmetries)) / len(asymmetries))
...
def APLR(EEG):
    """Computes the left-right and anterior-posterior power distributions measured along the scalp using
       ...
       Returns
       -------
       PLR : list Outputs the power distributions in the left-right direction by weighting the power
       with the channel positions.

       PAP : list Outputs the power distributions in the anterior-posterior direction by weighting the power with the
       channel positions.
    ...
    assert len(EEG) == 19, "Exception: Expected 19 channels"
    assert type(EEG) == np.ndarray, "Exception: Returned Type Mismatch"
    assert EEG.shape[1] >= 250 * 5, "Exception: To few datapoints recorded"

    # compute Welch's discrete power coefficients
    power = []
    for i in range(EEG.shape[0]):
        f, Pxx = welch(EEG[i], fs=c.FS, window='hann', noverlap=c.NOVERLAP, nfft=c.NFFT, nperseg=c.WINDOW)
        power.append(Pxx)
    ...
    PLR = np.array((P @ X) / P.sum(axis=1))
    PAP = np.array((P @ Y) / P.sum(axis=1))
...
def mNNC(EEG):
    ...
    -------
    AllCOH : list
         The final output is list of the mean coherence value for all 19 channels at each frequency.
