...
class Objective(object):
    ...
    def __call__(self, trial):
        num_dense_nodes = trial.suggest_categorical('num_dense_nodes', [3000, 2500, 2000, 1500, 1000])
        batch_size = trial.suggest_categorical('batch_size', [1, 5, 10, 32, 64])
        epochs_ = trial.suggest_categorical('num_epochs', [5, 10, 15, 20])
        learning_rate_ = trial.suggest_categorical('num_epochs', [0.00001, 0.00005, 0.0001, 0.0005, 0.001, 0.005])
        drop_out = trial.suggest_discrete_uniform('drop_out', 0.05, 0.2)
...
if __name__ == '__main__':
    ...
    maximum_time = 6 * 60 * 60 * 60  # seconds
    results_directory = '/'
    train_path = '/train/'
    valid_path = '/valid/'
    test_path = '/test/'
